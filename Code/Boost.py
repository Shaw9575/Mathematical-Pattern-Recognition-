...
def Ada(method,norm,selection):
    ...
    for train_index, dev_index in skf.split(dataset_train, dataset_label):
        ...
        if method == 1:
            AdaB.fit(train, label_train)
        if method == 2:
            AdaB.fit(train, label_train, sw)
        y_pred_train = AdaB.predict(train)
        ...
        y_pred_test = AdaB.predict(test)
        y_prob_pred_test = AdaB.predict_proba(test)
        ...
        # auc roc_curve
        auc = roc_auc_score(label_test, y_prob_pred_test[:, 1], average='weighted')
        print('auc = ', auc)
        auc_save.append(auc)
