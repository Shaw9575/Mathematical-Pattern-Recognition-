...
def LogicR(method,norm,selection):
    print('---------------------------------------------------------------------------------------------------')
    ...
    for train_index, dev_index in skf.split(dataset_train, dataset_label):
        ...
        if norm == 3:
            ...
            if method == 1:
                ...
                ratio = 1
            if method == 2:
                [ratio, sw] = weight.pre_select_data(dataset, norm,selection)
        ...
        LR = LogisticRegression(penalty='l1', C=10, class_weight={0:1,1:int(ratio)})
        ...
        y_prob_pred_test = LR.predict_proba(test)
